...
public interface LocalidadRepository extends JpaRepository<LocalidadesModel, LocalidadesId> {
    ...
    @Query("SELECT l FROM LocalidadesModel l WHERE l.id_entidad = :id_entidad AND l.id_municipio = :id_municipio")
    List<LocalidadesModel> findAllByIdEntidadAndIdMunicipio(Integer id_entidad, Integer id_municipio);
}
